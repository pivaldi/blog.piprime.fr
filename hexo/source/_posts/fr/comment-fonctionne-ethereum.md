...
**Ethereum est une plate-forme blockchain décentralisée et open source qui permet des contrats intelligents et des applications décentralisées (dApps)**.

* Lancé en 2015, Ethereum a été conçu pour répondre aux limites de Bitcoin en permettant des
...
* **Open** : n'importe qui peut participer au réseau sans avoir à demander la permission.
* **Sans frontières** : les transactions et les dApps fonctionnent à l’échelle mondiale sans intermédiaires.
* **Résistant à la censure** : aucune autorité centrale ne peut bloquer les transactions.
* **Immuable** : une fois validée les contrats ([smart contracts](https://www.binance.com/fr/academy/glossary/smart-contract)) et les transactions ne peuvent pas être altérées.
...
* Permettre aux développeurs de créer des dApps sans nécessiter une nouvelle blockchain.
* Introduire une forme d'argent programmable via des contrats intelligents (smart contracts).
...
* **Applications financiaires**
  ...
  * Il permet également la tokenisation des actifs, c'est à dire convertir des actifs du monde réel (des biens immobiliers par exemple) en jetons basés sur la blockchain.
* **Semi-Financial Applications**
  * Vérification d'identité : solutions d'identité numérique sécurisées par blockchain.
  * Intégrité des données: Garantir des dossiers infalsifiables pour les soins de santé, le gouvernement, les banques etc.
...
* Utilisé pour envoyer et recevoir de l'Ether (ETH) et interagir avec des smart contracts.
...
1. **Comptes de contrats (Smart Contracts)**
...
[La machine virtuelle Ethereum](https://www.binance.com/fr/academy/glossary/ethereum-virtual-machine-evm) est :
* Un moteur informatique décentralisé mondial qui traite les transactions Ethereum.
* L'EVM est un environnement d'exécution où s'exécute tous les smart contracts.
* Les smart contracts sont écrits en [Solidity](https://www.soliditylang.org/) et dans d'autres langages de programmation.
...
contrats intelligents. La plus petite unité d'ETH est Wei (1 ETH = $10^{18}$ Wei) et le réseau
exécute toutes les transactions internes en wei.  
Les principaux usage de l'Ether sont :
* **Frais de transaction** (Gas Fees) : les utilisateurs paient du [gas](https://www.binance.com/fr/academy/glossary/gas)[^1] en ETH pour exécuter les opérations.
* **Exécution de smart contracts** : ETH est nécessaire pour exécuter des applications décentralisées.
* [Staking](https://www.binance.com/fr/academy/articles/what-is-staking)[^2]
  dans [la preuve d'enjeu](https://www.binance.com/fr/academy/articles/proof-of-stake-explained) (Proof of Stake -- PoS --) : les validateurs stake (gage/prête) de l’ETH pour sécuriser le réseau.
...
1. Les validateurs gage un minimum de 32 ETH pour participer à la validation de bloc.
2. Un validateur est sélectionné au hasard (algorithme [RANDAO](https://github.com/randao/randao)) pour proposer le bloc suivant.
3. D'autres validateurs vérifient le block et parviennent à un consensus.
4. Les validateurs honnêtes gagnent des récompenses en ETH, tandis que les malhonnêtes risquent de perdre leur mise.

Le PoS réduit les exigences matérielles et réduit la consommation d’énergie. Cependant, il est
...
**Lorsqu'un point de contrôle est approuvé par la majorité, les transactions précédentes sont solidifiées, garantissant qu'elles ne peuvent plus être annulées.**
...
* Les frais de Gas permette aussi d'**éviter les attaques** de type spam en obligeant les utilisateurs à payer pour de la puissance de calcul.
...
Les utilisateurs fixent une limite de gas mesuré en Gwei qui détermine la quantité de travail de calcul qu'ils
...
* **Charge réseau** : plus il y a de d'activité, de demande, plus les frais augmentent.
* **La complexité de la transaction** : Les smart contracts coûtent plus que simple transfert d'ETH.
* **Pourboire prioritaire** : les utilisateurs peuvent payer plus de frais pour avoir une confirmation plus rapide de leur transaction.
...
* **[ERC-721](https://ethereum.org/fr/developers/docs/standards/tokens/erc-721/)**  
  Correspond à des tokens non fongible, il sont utilisés dans l'art numérique, les objets de collection et les actifs uniques
  ([CryptoPunks](https://www.cryptopunks.app/), [Bored Apes](https://fr.wikipedia.org/wiki/Bored_Ape_Yacht_Club)).  
...
* **[ERC-1155](https://ethereum.org/fr/developers/docs/standards/tokens/erc-1155/)**  
  ...
  fongibles dans un seul contrat et sont utilisés dans les actifs de jeu et dand le métaverse.
