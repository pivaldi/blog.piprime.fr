...
[^2]: $\sigma = \sqrt{\frac{1}{N - 1} \sum_{i=1}^{N} (r_i - \mu)^2}$ où : $\sigma$ est volatilité de l’actif, $r_i$ est le rendement de l’actif au jour $i$, $\mu$ est la moyenne des rendements, $N$ est le nombre total d’observations. Par exemple, si un actif a une volatilité de 20 % annualisée, cela signifie que ses rendements journaliers varient en moyenne de manière à générer une fluctuation de ±20 % sur un an.
